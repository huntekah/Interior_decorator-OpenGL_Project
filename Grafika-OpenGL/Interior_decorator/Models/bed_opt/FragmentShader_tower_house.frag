...
void main(){
	...
	vec4 MaterialAmbientColor = vec4(0.9,0.9,0.9,0) * MaterialDiffuseColor;
	vec4 MaterialSpecularColor = vec4(0.7,0.7,0.7,0);
	...
	color =  vec4(shine,0) * MaterialDiffuseColor + vec4(shine,0) * pow(cosTheta,3) + vec4(shine,0) * pow(cosAlpha,60) +
		...
		0.015 * MaterialSpecularColor * WhiteLight * LightPower * pow(cosAlpha,5) / (distance * distance) + 
		//White Diffusion
